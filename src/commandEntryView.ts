...
export class CommandEntryViewProvider implements vscode.WebviewViewProvider {
    ...
    private async prompt(command: string): Promise<void> {
        ...
        const genericSelection = await this.resolveGenericCommandSelectionRequest(normalizedCommand, this.getConnection());
        if (genericSelection !== undefined) {
            if (!genericSelection) {
                return;
            }

            normalizedCommand = genericSelection;
            const normalizedForDisplay = applyUserCommandLabel(normalizedCommand, labeledCommand.labelPrefix);
            if (normalizedForDisplay !== command) {
                this.post({ type: 'setCommand', command: normalizedForDisplay });
            }
    ...
    private async resolveGenericCommandSelectionRequest(command: string, connection?: IBMi): Promise<string | null | undefined> {
    ...
    private async run(
        ...
        const genericSelection = await this.resolveGenericCommandSelectionRequest(command, connection);
        if (genericSelection !== undefined) {
            return;
        }
