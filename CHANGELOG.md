...
- New Command Entry SQL Job startup script and log is available so you can run specific SQL or CL commands when initially connecting Command Entry to the host SQL job.
  ...
  - Startup script entries use a semicolon-terminated statement model, with explicit `SQL:` and `CL:` prefixes supported for clarity. The default is `CL:` when not specified, expect when the statement is `SET`, `VALUES` or `SELECT` in that case `SQL:` is assumed.
...
- **`Case=MIXED` / `Case=MONO` attribute support**: The prompter and formatter now honour the `Case` attribute from Command Definition XML. Parameters marked `Case=MIXED` preserve the user's entered capitalization exactly as typed — the value is never uppercased when the command is returned to the source editor. Parameters marked `Case=MONO` (the default for most parameters) continue to allow case conversion per the existing formatter case settings.

- **`DspInput=NO` attribute support**: Parameters whose XML carries `DspInput="NO"` are now rendered as password fields (`type="password"`), masking the entered value. This also prevents the field from being promoted to a multi-line textarea regardless of its declared length, consistent with IBM i green-screen behaviour for non-display inputs.

- **`InlPmtLen` attribute support**: The `InlPmtLen` attribute from Command Definition XML is now used to set each input field's initial display width. When present, it takes precedence over the storage `Len` for sizing purposes — for example, a field with `Len="128"` but `InlPmtLen="PWD"` (or `InlPmtLen="*PWD"`) renders at a compact 10-character width rather than the full storage length. `InlPmtLen="*CALC"` or an absent attribute defers to `Len`.
